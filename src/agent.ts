...
async function main() {
  ...
  resetGamesLog();
  const games: any[] = [];
  const loggedOrdinals = new Set<number>();
  ...
  for (let iter = 0; iter < MAX_ITERS; iter++) {
    ...
    if (t === "MOVE_REQUIRED") {
      ...
      loggedOrdinals.add(rec.gameOrdinal);
      games.push(rec);
      console.log(
      ...
      // The final game (G15, Centauri Battlecruiser) arrives as ATTEMPT_COMPLETED
      // directly — there is no trailing GAME_COMPLETED for it — so it was never
      // summarized/archived (only 14 of 15 logged). Capture it from lastState
      // (its last MOVE_REQUIRED: opponent, our shots, incoming) before printing,
      // so Centauri finally gets data for determinism/fingerprint analysis.
      const finalOrd = lastState?.gameOrdinal;
      if (lastState && finalOrd != null && !loggedOrdinals.has(finalOrd)) {
        const rec = summarizeGame(finalOrd, lastState, resp);
        appendGame(rec);
        appendDetail(rec.gameOrdinal, rec.opponentClass, lastState, lastPlacements);
        loggedOrdinals.add(finalOrd);
        games.push(rec);
        console.log(
          `\n✓ Game ${rec.gameOrdinal} (final) captured — vs ${rec.opponent} shots=${rec.shots} hits=${rec.hits}`,
        );
      }
